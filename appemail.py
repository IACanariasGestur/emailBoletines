...
Original file is located at
    https://colab.research.google.com/drive/1CZa5BCJ9q8O2YZoKBdbzI8ik3X9VWQSx
"""
...
st.title("📰 Resumen automático de Boletines Oficiales (Canarias)")
st.info("Busca legislación y anuncios relevantes en BOE, BOC, BOP LP, BOP SCTF.\n\n"
...
st.markdown("---")
st.header("🔎 Buscar anuncios relevantes")
...
if st.button("Buscar boletines oficiales"):
    ...
    if documentos:
        ...
        # Convertimos para mostrar tabla (quitando campos largos)
        tabla = []
        for d in documentos:
...
st.markdown("---")
st.header("✉️ Enviar resumen por email")

if "resultados" in st.session_state and st.session_state.resultados:
    with st.expander("Configurar envío de email"):
        remitente = st.text_input("Correo de envío (Gmail)", "canariasgestur@gmail.com")
        clave_app = st.text_input("Clave de aplicación Gmail", type="password")
        destinatarios = st.session_state.destinatarios

        if st.button("Enviar resumen"):
            if not remitente or not clave_app or not destinatarios:
